...
class VisualizeImageMaximizeFmap(object):
    ...
    def __init__(self,model, pic_shape):
        '''
    ...
    def create_iterate(self,input_img, layer_output,filter_index):
        ...
        iterate = tf.function([input_img, tf.learning_phase()], [loss, grads])
        return(iterate)
    ...
    def plot_images_wrapper(self,argimage,n_row = 8, scale = 1, save_path="/tmp/tensorflow/mnist/vis"
    ,supress=True):
        '''
        ...
        for layer_name in layer_names:
            ...
            if save_path:
                ...
                "vis-{}".format(layer_name)))
            
            # Whether to supress plt
            if not supress:
                plt.show()
            else:
                plt.close()
